...
- (BOOL)adm_createFilesAndDirectoriesAtPath:(NSString *)path withTarObject:(id)object size:(unsigned long long)size error:(NSError * __autoreleasing *)error;
+ (char)adm_typeForObject:(id)object atOffset:(unsigned long long)offset;
+ (NSString*)adm_nameForObject:(id)object atOffset:(unsigned long long)offset;
+ (unsigned long long)adm_sizeForObject:(id)object atOffset:(unsigned long long)offset;
- (void)adm_writeFileDataForObject:(id)object atLocation:(unsigned long long)location withLength:(unsigned long long)length atPath:(NSString*)path;
+ (NSData*)adm_dataForObject:(id)object inRange:(NSRange)range orLocation:(unsigned long long)location andLength:(unsigned long long)length;
@end
...
+ (BOOL)untarFileAtPath:(NSString *)path error:(NSError * __autoreleasing *)error
{
    ...
    if ([fm fileExistsAtPath:path]) {
        ...
        success = [[NSFileManager defaultManager] adm_createFilesAndDirectoriesAtPath:parentPath
...
- (BOOL)adm_createFilesAndDirectoriesAtPath:(NSString *)path withTarObject:(id)object size:(unsigned long long)size error:(NSError * __autoreleasing *)error
{
    ...
    while (location<size) {
        ...
        @autoreleasepool {
            ...
            switch ([[self class] adm_typeForObject:object atOffset:location]) {
                case '0': // It's a File
                {
                    NSString* name = [[self class] adm_nameForObject:object atOffset:location];

                    NSString *filePath = [path stringByAppendingPathComponent:name]; // Create a full path from the name
                    
                    unsigned long long size = [[self class] adm_sizeForObject:object atOffset:location];
                    ...
                    [self adm_writeFileDataForObject:object atLocation:(location+kTarBlockSize) withLength:size atPath:filePath];
                    break;
                }
...
{
    char type;
    memcpy(&type, [self adm_dataForObject:object
...
+ (NSString *)adm_nameForObject:(id)object atOffset:(unsigned long long)offset
{
    ...
    memcpy(&nameBytes, [self adm_dataForObject:object
...
+ (unsigned long long)adm_sizeForObject:(id)object atOffset:(unsigned long long)offset
{
    ...
    memcpy(&sizeBytes, [self adm_dataForObject:object
